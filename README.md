...
`0.9.7` 将主窗口工作区改为“VPN 连接 / 本地路由”左右切换；切换到“本地路由”后，规则列表、网卡、DNS 和保存操作都直接在主窗口中完成，不再弹出独立配置窗口。菜单栏和应用菜单也会切换主窗口到本地路由工作区。

`0.9.8` 为规则增加当前规则目录内的隐藏 `.backup/` 备份。保存规则时同步备份；如果主规则目录被外部清空，应用下次打开本地路由工作区时会自动恢复有效备份，避免后台仍有路由而界面列表为空。旧版并列的 `rules-backup/` 会自动迁移到这里。
